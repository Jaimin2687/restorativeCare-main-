...
// --- DB Connection ---
$DB_HOST = 'localhost';
$DB_PORT = '3306'; // Use your actual port
$DB_USER = 'root';
$DB_PASS = '';
...
function buildFeed(data = notifications){
  ...
  if (currentSort === 'time') list.sort((a,b)=> new Date(b.created_at || 0) - new Date(a.created_at || 0));
...
function smartSnooze(id){
  ...
  const d = new Date(n.created_at || new Date());
  d.setMinutes(d.getMinutes() + 30);
  n.created_at = d.toISOString();
  buildFeed();
...
function showExplain(n){
  ...
  } else if (n.title && n.title.toLowerCase().includes('potassium')) {
...
function selectNotification(id){
  ...
  // Build detail HTML
  let html = `<div class="font-semibold">${n.user_name || 'Notification'}</div>`;
  html += `<div class="muted text-sm">${niceDateTime(n.created_at)} • Urgency ${n.urgency}%</div>`;
  html += `<div class="mt-3">${n.message}</div>`;
...
function avatarSpeak(){
  ...
  const top = [...notifications].sort((a,b)=> (b.urgency || '').localeCompare(a.urgency || '')).slice(0,3);
  const text = top.map(t => `${t.user_name || 'Notification'}. ${t.message}`).join(' Next: ');
  if ('speechSynthesis' in window) {
    const patientName = <?php echo json_encode($patientName); ?>;
    const u = new SpeechSynthesisUtterance(`Hello ${patientName}. ${text}`);
    u.lang = 'en-US';
    u.rate = 0.95;
...
/* Mood aware toggle handling */
document.getElementById('moodAware').addEventListener('change', (e)=> {
  moodAware = e.target.checked;
  showToast(`Mood awareness ${moodAware ? 'enabled' : 'disabled'} (demo).`);
});
...
document.addEventListener('DOMContentLoaded', ()=> {
  ...
  announce('You have ' + notifications.length + ' notifications. ' + 
    (notifications.filter(n => parseInt(n.urgency) >= 80).length) + ' critical.');
});
